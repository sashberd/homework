...
directiveModule.directive('ngDropdownMultiselect', ['$filter', '$document', '$compile', '$parse', function ($filter, $document, $compile, $parse) {
    return {
        ...
        template: function (element, attrs) {
            ...
            template += '<li ng-if="settings.columnsCount>0 && options.length>0">  <label class="multiboxOptionHeader" set-header-style header-index="{{$index}}" ng-repeat="(key, value) in settings.headers">{{key}}</label>  ';
            template += '<li ng-if="!options.length">  <label class="multiboxOptionHeader">No matches found </label>  ';
            if (groups) {
            ...
            if (checkboxes) {
                template += '<div class="checkbox"><label><input class="checkboxInput" type="checkbox" ng-click="checkboxClick($event, getPropertyForObject(option,settings.idProp))" ng-checked="isChecked(getPropertyForObject(option,settings.idProp))" /> <span  ng-bind-html="getPropertyForObject(option, settings.displayProp) | html"></span></div></label></a>';
                ...
                template += '<span data-ng-class="{\'glyphicon glyphicon-ok\': isChecked(getPropertyForObject(option,settings.idProp))}"> </span>  <span  ng-bind-html="getPropertyForObject(option, settings.displayProp) | html"></span></a>';
        ...
        link: function ($scope, $element, $attrs) {
            ...
            function getFindObj(id) {
                var findObj = {};
                id = Object.prototype.toString.call(id) === '[object String]' ? angular.element(id).find('label')[1].innerText : id;
            ...
            $scope.getPropertyForObject = function (object, property) {
                ...
                if (angular.isDefined(object) && object.hasOwnProperty(property)) {
                    var divTemplate = angular.element('<div class="multipleHtmlOptionDiv"></div>');
                    if ($scope.settings.multiColumn) {
                        Object.values($scope.settings.headers).forEach(function (item) {
                            divTemplate.append('<label>' + object[item] + '</label>')
                        });
                        result = divTemplate[0].outerHTML;
                    }
                    else {
                ...
                var temp = document.body.appendChild(divTemplate[0]);
                var newOptionWidth = window.getComputedStyle(temp).width;
                $scope.multiselectULMinWidth = Math.max(parseInt($scope.multiselectULMinWidth), parseInt(newOptionWidth) + 75);
                document.body.removeChild(divTemplate[0]);
                return result;
            };
            ...
            $scope.selectAll = function () {
                ...
                $scope.externalEvents.onSelectAll($scope);
            ...
            $scope.deselectAll = function (dontSendEvent) {
                ...
                if (!dontSendEvent) {
                    $scope.externalEvents.onDeselectAll(this);
                }
