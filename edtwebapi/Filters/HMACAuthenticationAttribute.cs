    ...
    public class HMACAuthenticationAttribute : Attribute, IAuthenticationFilter
    {
        ...
        public Task AuthenticateAsync(HttpAuthenticationContext context, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    dynamic header = JsonConvert.DeserializeObject(req.Headers.GetValues("Authorization").ToList().FirstOrDefault());

                    req.Headers.Authorization = new AuthenticationHeaderValue(header.scheme.Value, header.parameter.Value);
        ...
        private async Task<bool> isValidRequest(HttpRequestMessage req, string APPId, string incomingBase64Signature, string nonce, string requestTimeStamp)
        {
            ...
            var secretKeyBytes = Convert.FromBase64String(ConfigurationManager.AppSettings["ApiKey"]);
        ...
        private bool isReplayRequest(string nonce, string requestTimeStamp)
        {
            ...
            TimeSpan requestTs = new DateTime(Convert.ToInt64(requestTimeStamp)) - epochStart;

            var serverTotalSeconds = Convert.ToUInt64(currentTs.TotalSeconds);
            var requestTotalSeconds = Convert.ToUInt64(requestTs.TotalSeconds);
